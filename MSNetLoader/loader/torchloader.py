...
if __name__ == '__main__':
    pept = AlphaPeptDeepConverter(
        "/mnt/daicx/pvc-afbfaa68-aa52-416c-b273-64fb016fd745/msnet_paper/released_data_v3/PXD012636_Danio_rerio-MSNet.parquet")
    precursor_df, fragment_df = pept.convert_parquet_to_training_format()
    print(precursor_df)
    print(fragment_df)

    md = MS2TorchDataset(precursor_df, fragment_df)

    lengths = md.dataset["nAA"]

    batch_sampler = LengthExactSampler(
        lengths=lengths,
        batch_size=32,
        shuffle=True
    )
